    ...
    <div class="search-container">
        <div class="search-wrapper relative flex-grow max-w-2xl">
            <div class="search-input-group relative">
                ...
                <div v-if="isLoading" class="loading-spinner absolute right-4 top-1/2 transform -translate-y-1/2 w-4 h-4"></div>
            </div>

            <!-- Autocomplete Dropdown -->
            <div
                v-if="showSuggestions && (recentSearches.length > 0 || filteredSuggestions.length > 0 || query.trim())"
            ...
            >
                <!-- Loading State -->
                <div v-if="isLoading && query.trim()" class="px-4 py-4 flex items-center justify-center gap-2 text-gray-500 text-sm">
                    <div class="spinner-small w-3 h-3"></div>
                    <span>Finding suggestions...</span>
                </div>

                <!-- Recent Searches -->
                <div v-else-if="!query.trim() && recentSearches.length > 0" class="border-b border-gray-100">
                    <p class="px-4 py-2 text-xs font-semibold text-gray-500 uppercase tracking-wider">Recent</p>
                    <button
                        v-for="(search, index) in recentSearches"
                        :key="`recent-${search}`"
                        :class="['block w-full text-left px-4 py-2 hover:bg-gray-50 transition-colors text-sm text-gray-700', { 'bg-blue-50': selectedIndex === index }]"
                ...
                <template v-else-if="filteredSuggestions.length > 0">
                    <!-- Books Section -->
                    <div v-if="bookSuggestions.length > 0" class="border-b border-gray-100">
                        <p class="px-4 py-2 text-xs font-semibold text-gray-500 uppercase tracking-wider">Books</p>
                        <button
                            v-for="(item, index) in bookSuggestions"
                            :key="`book-${item.id}`"
                            :class="['w-full text-left px-4 py-2 hover:bg-gray-50 transition-colors flex items-center gap-3', { 'bg-blue-50': selectedIndex === index }]"
                        ...
                        >
                            ...
                            <div class="flex-1 min-w-0">
                                <div class="text-sm text-gray-900 truncate" v-html="highlightMatch(item.title)"></div>
                                <div class="text-xs text-gray-500">by {{ item.author?.name || 'Unknown' }}</div>
                            </div>
                    ...
                    <!-- Authors Section -->
                    <div v-if="authorSuggestions.length > 0" class="border-b border-gray-100">
                        <p class="px-4 py-2 text-xs font-semibold text-gray-500 uppercase tracking-wider">Authors</p>
                        <button
                            v-for="(item, index) in authorSuggestions"
                            :key="`author-${item.id}`"
                            :class="['w-full text-left px-4 py-2 hover:bg-gray-50 transition-colors flex items-center gap-3', { 'bg-blue-50': selectedIndex === bookSuggestions.length + index }]"
                        ...
                        >
                            ...
                            <div class="flex-1 min-w-0">
                                <div class="text-sm text-gray-900 truncate" v-html="highlightMatch(item.author?.name)"></div>
...
const highlightMatch = (text: string): string => {
    ...
    return text.replace(regex, '<mark class="font-semibold text-orange-600">$1</mark>');
};
