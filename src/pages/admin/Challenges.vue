      ...
      <table class="challenges-table">
        ...
        <tbody>
          <tr v-for="c in challenges" :key="c.id" class="challenge-row">
            <td>
              <div class="challenge-icon" :style="{ background: c.color }" v-html="c.icon"></div>
            </td>
            <td class="challenge-title"><strong>{{ c.title }}</strong></td>
            <td class="challenge-desc">{{ c.description }}</td>
            <td>{{ c.targetBooks }} books</td>
            <td>{{ c.deadline ? new Date(c.deadline).toLocaleDateString() : 'No deadline' }}</td>
            <td>
            ...
            <td>
              <div class="actions">
                ...
                <button class="action-btn delete" title="Delete" @click="confirmDelete(c)">
          ...
          <tr v-if="!challenges.length">
            <td colspan="7" class="empty-state">No challenges yet. Click "Add Challenge" to create one.</td>
          </tr>
        </tbody>
      </table>
    </div>
      ...
      <div class="modal">
        ...
        <div class="modal-body">
          <div class="form-group">
            <label>Title</label>
            <input v-model="form.title" type="text" class="form-input" placeholder="e.g. 2025 Reading Challenge" />
          </div>
          <div class="form-group">
            <label>Description</label>
            <textarea v-model="form.description" class="form-input" rows="3" placeholder="Describe the challenge"></textarea>
          </div>
          <div class="form-row">
            <div class="form-group">
              <label>Target Books</label>
              <input v-model.number="form.targetBooks" type="number" min="1" class="form-input" />
            </div>
            <div class="form-group">
            ...
            <div class="form-group">
              ...
              <div class="color-picker">
                <button
                  ...
                  @click="form.color = c"
                ></button>
                <input v-model="form.color" type="text" class="form-input color-input" placeholder="#hex" />
              </div>
            </div>
          </div>
          <div class="form-group">
            ...
            <div class="icon-picker">
              <button
                v-for="ic in presetIcons" :key="ic.id"
                class="icon-option"
            ...
            <details class="custom-icon-details">
              ...
              <textarea v-model="form.icon" class="form-input form-input-mono" rows="3" placeholder='<svg ...>...</svg>'></textarea>
            </details>
...
export default {
  ...
  data() {
    return {
      ...
      presetIcons: [
        { id: 'book', label: 'Book', svg: '<svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"><path d="M4 19.5A2.5 2.5 0 016.5 17H20"/><path d="M6.5 2H20v20H6.5A2.5 2.5 0 014 19.5v-15A2.5 2.5 0 016.5 2z"/></svg>' },
        { id: 'star', label: 'Star', svg: '<svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"><polygon points="12 2 15.09 8.26 22 9.27 17 14.14 18.18 21.02 12 17.77 5.82 21.02 7 14.14 2 9.27 8.91 8.26 12 2"/></svg>' },
        { id: 'trophy', label: 'Trophy', svg: '<svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"><path d="M6 9H4.5a2.5 2.5 0 010-5H6"/><path d="M18 9h1.5a2.5 2.5 0 000-5H18"/><path d="M4 22h16"/><path d="M10 14.66V17c0 .55-.47.98-.97 1.21C7.85 18.75 7 20.24 7 22"/><path d="M14 14.66V17c0 .55.47.98.97 1.21C16.15 18.75 17 20.24 17 22"/><path d="M18 2H6v7a6 6 0 0012 0V2z"/></svg>' },
        { id: 'target', label: 'Target', svg: '<svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"><circle cx="12" cy="12" r="10"/><circle cx="12" cy="12" r="6"/><circle cx="12" cy="12" r="2"/></svg>' },
        { id: 'pen', label: 'Write', svg: '<svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"><path d="M17 3a2.85 2.83 0 114 4L7.5 20.5 2 22l1.5-5.5L17 3z"/></svg>' },
        { id: 'stack', label: 'Books', svg: '<svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"><path d="M12 6.253v13m0-13C10.832 5.477 9.246 5 7.5 5S4.168 5.477 3 6.253v13C4.168 18.477 5.754 18 7.5 18s3.332.477 4.5 1.253m0-13C13.168 5.477 14.754 5 16.5 5c1.747 0 3.332.477 4.5 1.253v13C19.832 18.477 18.247 18 16.5 18c-1.746 0-3.332.477-4.5 1.253"/></svg>' },
        { id: 'medal', label: 'Medal', svg: '<svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"><circle cx="12" cy="8" r="5"/><path d="M12 13l-2.5 6 2.5-1 2.5 1-2.5-6z"/></svg>' },
        { id: 'fire', label: 'Streak', svg: '<svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"><path d="M12 2l1.5 3.5L17 7l-3.5 1.5L12 12l-1.5-3.5L7 7l3.5-1.5L12 2z"/><path d="M12 12l1.5 3.5L17 17l-3.5 1.5L12 22l-1.5-3.5L7 17l3.5-1.5L12 12z"/></svg>' },
        { id: 'heart', label: 'Heart', svg: '<svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"><path d="M20.84 4.61a5.5 5.5 0 00-7.78 0L12 5.67l-1.06-1.06a5.5 5.5 0 00-7.78 7.78l1.06 1.06L12 21.23l7.78-7.78 1.06-1.06a5.5 5.5 0 000-7.78z"/></svg>' },
        { id: 'crown', label: 'Crown', svg: '<svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"><path d="M2 4l3 12h14l3-12-6 7-4-7-4 7-6-7z"/><path d="M3 20h18"/></svg>' },
        { id: 'sparkles', label: 'Special', svg: '<svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"><path d="M12 3l1.5 4.5L18 9l-4.5 1.5L12 15l-1.5-4.5L6 9l4.5-1.5L12 3z"/><path d="M5 15l1.5 3L10 19l-3.5 1L5 23l-1.5-3L0 19l3.5-1L5 15z"/><path d="M19 10l1 2 2 1-2 1-1 2-1-2-2-1 2-1 1-2z"/></svg>' },
        { id: 'compass', label: 'Explore', svg: '<svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"><circle cx="12" cy="12" r="10"/><polygon points="16.24 7.76 14.12 14.12 7.76 16.24 9.88 9.88 16.24 7.76"/></svg>' },
        { id: 'clock', label: 'Timed', svg: '<svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"><circle cx="12" cy="12" r="10"/><polyline points="12 6 12 12 16 14"/></svg>' },
        { id: 'users', label: 'Community', svg: '<svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"><path d="M17 21v-2a4 4 0 00-4-4H5a4 4 0 00-4 4v2"/><circle cx="9" cy="7" r="4"/><path d="M23 21v-2a4 4 0 00-3-3.87"/><path d="M16 3.13a4 4 0 010 7.75"/></svg>' },
        { id: 'leaf', label: 'Nature', svg: '<svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"><path d="M11 20A7 7 0 019.8 6.9C15.5 4.9 17 3.5 19 2c1 2 2 4.5 2 8 0 5.5-4.78 10-10 10z"/><path d="M2 21c0-3 1.85-5.36 5.08-6C9.5 14.52 12 13 13 12"/></svg>' },
      ],
    }
  },
  created() {
