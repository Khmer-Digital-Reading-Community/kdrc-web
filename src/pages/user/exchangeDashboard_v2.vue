<template>
  <div
    class="min-h-screen bg-[#F7F1E6] py-12 px-4 sm:px-6 lg:px-8 font-sans text-gray-900"
  >
    <!-- Header -->
    <div class="mb-8">
      <p class="text-3xl sm:text-4xl font-bold mt-2">Your Exchange Dashboard</p>

      <p class="text-gray-500 text-lg mt-2 text-[17px] pt-2">
        Manage your book listings, track exchange requests, and connect with
        fellow readers in one place.
      </p>
    </div>

    <div
      v-if="loading"
      class="max-w-7xl mx-auto py-20 text-center text-gray-500"
    >
    ...
    <div v-else class="max-w-7xl mx-auto space-y-12">
      <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
        <div
          class="bg-white p-6 rounded-2xl shadow-sm border border-gray-100 flex items-center gap-4 relative overflow-hidden"
        >
          <div class="absolute left-0 top-0 bottom-0 w-1.5 bg-[#8B5CF6]"></div>
          <div
            class="w-12 h-12 bg-purple-50 rounded-full flex items-center justify-center text-[#8B5CF6]"
          >
            <svg
              class="w-6 h-6"
              fill="none"
        ...
        >
          ...
          <div
            class="w-12 h-12 bg-yellow-50 rounded-full flex items-center justify-center text-[#B58E2C]"
          >
            <svg
              class="w-6 h-6"
              fill="none"
        ...
        >
          ...
          <div
            class="w-12 h-12 bg-orange-50 rounded-full flex items-center justify-center text-[#78350F]"
          >
            <svg
              class="w-6 h-6"
              fill="none"
      ...
      <div>
        <div class="flex items-center justify-between mb-6">
          ...
          <router-link
            to="/post-exchange"
            class="text-sm font-medium text-[#8B5CF6] hover:text-[#7C3AED]"
          >
            Post New Book →
          </router-link>
        </div>
        ...
        <div
          ...
          class="rounded-3xl border border-dashed border-gray-200 bg-white p-10 text-center text-gray-500"
        >
          You have not posted any books yet.
          <router-link
            to="/post-exchange"
            class="mt-3 block font-semibold text-[#8B5CF6] hover:underline"
          >
        ...
        <div v-else class="grid grid-cols-2 md:grid-cols-4 gap-6">
          <div
            v-for="book in myListings"
            :key="book.id"
            class="bg-white p-4 rounded-3xl shadow-sm border border-gray-100 flex flex-col"
          >
            <router-link
              :to="{ name: 'book-exchange-detail-v2', params: { id: book.id } }"
              class="block"
            >
              <div
                class="h-64 bg-gray-100 rounded-xl overflow-hidden mb-4 relative"
              >
                <img
                  :src="book.imageUrl"
                  :alt="book.title"
              ...
              <p
                class="text-[10px] font-extrabold text-[#B58E2C] uppercase tracking-wider mb-1"
              >
                {{ book.listingStatus || "ACTIVE" }}
              </p>
              <h4
                class="font-bold text-gray-900 text-lg leading-tight line-clamp-1 mb-1"
              >
                {{ book.title }}
              </h4>
        ...
        >
          <div class="flex border-b border-gray-100 px-6 pt-4 gap-8">
            ...
            >
              ...
              <div
                ...
                class="absolute bottom-0 left-0 right-0 h-0.5 bg-[#8B5CF6] rounded-t-full"
              ></div>
            </button>
          </div>
          ...
          <div v-if="activeTab === 'incoming'" class="overflow-x-auto">
            <div
              ...
              class="px-8 py-12 text-center text-gray-500"
            >
                ...
                >
                  ...
                  <td class="px-6 py-4">
                    <div class="flex items-center gap-4">
                      ...
                      >
                        <img
                          :src="req.offeredExchange?.imageUrl"
                          class="w-full h-full object-cover"
                        />
                      </div>
                      <div>
                        <p class="font-bold text-gray-900 text-sm">
                          {{ req.offeredExchange?.title }}
                        </p>
                        <p class="text-xs text-gray-500">
                          {{ req.offeredExchange?.author }}
                        </p>
                      </div>
                    </div>
                  </td>
                  ...
                  <td class="px-6 py-4">
                    ...
                    >
                      <span
                        class="status-dot"
                        :class="getStatusDotClasses(req.status)"
                      ></span>
                  ...
                  <td class="px-8 py-4 text-right">
                    ...
                    >
                      <button
                        @click="rejectIncomingRequest(req.id)"
                        class="w-8 h-8 rounded-full border border-gray-200 flex items-center justify-center text-red-500 hover:bg-red-50 hover:border-red-100 transition-colors"
                      ...
                      <button
                        @click="acceptIncomingRequest(req.id)"
                        class="w-8 h-8 rounded-full bg-[#8B5CF6] flex items-center justify-center text-white hover:bg-[#7C3AED] transition-colors shadow-sm"
                ...
                >
                  <td class="px-8 py-4">
                    <div class="flex items-center gap-4">
                      ...
                      >
                        <img
                          :src="proposal.exchange?.imageUrl"
                          class="w-full h-full object-cover"
                        />
                      </div>
                      <div>
                        <p class="font-bold text-gray-900 text-sm">
                          {{ proposal.exchange?.title }}
                        </p>
                        <p class="text-xs text-gray-500">
                          {{ proposal.exchange?.author }}
                        </p>
                      </div>
                    </div>
                  </td>
                  <td class="px-6 py-4">
                    <div class="flex items-center gap-4">
                      ...
                      >
                        <img
                          :src="proposal.offeredExchange?.imageUrl"
                          class="w-full h-full object-cover"
                        />
                      </div>
                      <div>
                        <p class="font-bold text-gray-900 text-sm">
                          {{ proposal.offeredExchange?.title }}
                        </p>
                        <p class="text-xs text-gray-500">
                          {{ proposal.offeredExchange?.author }}
                        </p>
                      </div>
                    </div>
                  </td>
                  <td class="px-6 py-4">
                    <span class="font-bold text-gray-900 text-sm">{{
                      proposal.exchange?.owner?.name || "Owner"
                    }}</span>
                  </td>
                  <td class="px-6 py-4 text-sm text-gray-500">
                  ...
                  <td class="px-6 py-4">
                    ...
                    >
                      <span
                        class="status-dot"
                        :class="getStatusDotClasses(proposal.status)"
                      ></span>
...
const tabs = computed(() => [
  {
    ...
    name: `Incoming Requests (${incomingRequests.value.length})`,
  },
  {
    id: "proposals" as const,
    name: `My Proposals (${myProposals.value.length})`,
  },
]);
...
onMounted(async () => {
  if (!authState.token.value) {
    router.push({
      path: "/login",
      query: { redirect: "/exchange-dashboard-v2" },
    });
    return;
  }
