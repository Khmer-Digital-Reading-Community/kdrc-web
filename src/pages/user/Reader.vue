<template>
    <div
        class="reader-shell bg-gray-100 h-screen w-full flex items-center justify-center p-4 md:p-8"
    >
        <!-- Loading State -->
        <div
        ...
        >
            ...
            <router-link
                to="/user/home"
                class="text-red-600 hover:text-red-700 font-semibold underline"
            >
                ← Back to Home
            </router-link>
        </div>
        ...
        <div
            ...
            class="bg-white w-full max-w-6xl h-full rounded-lg shadow-2xl flex flex-col overflow-hidden border border-gray-200 ring-4 ring-blue-500/10"
        >
            ...
            <!-- Header -->
            <header
                class="bg-[#fcfcfc] border-b border-gray-100 flex items-center justify-between px-4 py-3 shrink-0"
            >
                <div class="flex items-center gap-4">
                    <router-link
                        to="/user/home"
                        class="w-8 h-8 bg-white border border-gray-200 rounded shadow-sm flex items-center justify-center text-gray-600 hover:bg-gray-50 transition-colors group"
                        title="Back to Home"
                    >
                        <svg
                    ...
                    <div class="flex flex-col gap-0.5">
                        ...
                        <p
                            class="text-sm font-bold text-gray-800 truncate max-w-xs"
                        >
                ...
                <div class="flex items-center gap-2">
                    ...
                    <div class="text-right mr-2">
                        ...
                        >
                            <div
                                class="h-full bg-[#B4690E] rounded-full transition-all duration-300"
                                :style="{ width: progressPercentage + '%' }"
                            ></div>
                        </div>
                    </div>
                    <button
                        class="w-8 h-8 bg-white border border-gray-200 rounded shadow-sm flex items-center justify-center text-gray-600 hover:bg-gray-50 transition-colors"
                        title="Bookmark"
                    >
                        <svg
                            class="w-4 h-4"
            ...
            <main class="flex-1 overflow-y-auto relative" ref="mainContentRef">
                <div class="max-w-3xl mx-auto px-8 py-12 md:py-16 relative">
                    ...
                    <div class="text-center mb-16 relative">
                        <span
                            class="text-[10px] font-bold text-[#B4690E] tracking-[0.2em] uppercase"
                        >
                            Chapter {{ currentChapter.chapterNumber }}
                        </span>
                        <h2
                            class="text-3xl text-gray-800 mt-2 font-serif font-semibold leading-tight"
                        >
                            {{ currentChapter.title }}
                        </h2>
                        ...
                        <!-- Chapter metadata -->
                        <div
                            v-if="currentChapter.wordCount"
                            class="flex items-center justify-center gap-4 mt-4 text-xs text-gray-400"
                        >
                            <span>{{ currentChapter.wordCount }} words</span>
                            <span>•</span>
                            <span
                                >~{{
                                    currentChapter.readingTimeMinutes || 10
                                }}
                                min read</span
                            >
                        </div>
                        ...
                        <!-- Settings button -->
                        <button
                            class="absolute top-0 right-0 md:-right-16 lg:-right-24 w-8 h-8 bg-white border border-gray-200 rounded shadow-sm hidden sm:inline-flex items-center justify-center text-gray-600 hover:bg-gray-50 transition-colors group"
                    ...
                    <!-- Chapter Content -->
                    <article
                        class="text-[#6b7280] text-base md:text-[17px] leading-[1.8] space-y-8 font-serif prose prose-sm max-w-none"
                    >
                        <div
                            v-html="renderContent(currentChapterContent)"
                            class="prose prose-lg max-w-none reader-content"
                        ></div>
                    </article>

                    <!-- Error notification in content area -->
                    <div
                        v-if="error && currentChapter"
                        class="mt-12 p-4 bg-yellow-50 border border-yellow-200 rounded-lg text-sm text-yellow-700"
                    >
                        {{ error }}
                    </div>
                </div>
            ...
            <!-- Footer Navigation -->
            <footer
                class="bg-[#fcfcfc] border-t border-gray-100 px-6 py-4 shrink-0"
            >
                <div class="flex items-center justify-between gap-4">
                    ...
                    <div class="text-center flex-1">
                        ...
                        <div
                            class="mt-1.5 w-full h-2 bg-gray-200 rounded-full overflow-hidden"
                        >
                            <div
                                ...
                                :style="{
                                    ...
                                    background:
                                        'linear-gradient(to right, #093A3F, #B4690E)',
                                }"
                            ></div>
                        </div>
                    </div>
                ...
                >
                    <details class="group cursor-pointer">
                        <summary
                            class="text-xs font-semibold text-gray-600 hover:text-gray-800 flex items-center gap-2"
                        >
                            <svg
                        ...
                        <div
                            class="mt-2 grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-1.5 max-h-40 overflow-y-auto"
                        >
                            <button
...
const handleProgressSaved = (progress: {
    ...
    if (currentChapter.value) {
        saveChapterProgress(
            ...
            Math.round(
                (progress.scroll / 100) *
                    (currentChapter.value.wordCount || 2500),
            ),
        );
